...
import numpy as np
import pandas as pd
from utils import get_topic_diversity, InvertedRBO

import file_handling as fh
from scholar_vic import Scholar
import torch
...
def main(args):
    ...
    parser.add_option('-k', dest='n_topics', type=int, default=50,
    ...
    parser.add_option('--epochs', type=int, default=1000,
    ...
    parser.add_option('--o', dest='output_dir', type=str, default='output/vic',
    ...
    parser.add_option('--retrain', type=int, default=0, help='train with training set and valid set or not')
    parser.add_option('--model', type=str, default='contrastiveScholar')
    parser.add_option('--topk', type=int, default=15)
    parser.add_option('--sim_coeff', type=float, default=80.0)
    parser.add_option('--std_coeff', type=float, default=80.0)
    parser.add_option('--cov_coeff', type=float, default=90.0)
    parser.add_option('--deepvic', type=int, default=0)
    parser.add_option('--deepvc', type=int, default=0)
    parser.add_option('--cl', type=int, default=0)
    parser.add_option('--positive', type=str, default=None)
    options, args = parser.parse_args(args)
    input_dir = args[0]
    options.input_dir = input_dir

    device = torch.device("cuda:" + get_most_freemem_gpu() if torch.cuda.is_available() else "cpu")
    ...
    if '20ng' in input_dir:
        ...
        dataset_name = 'wiki'
        if 'contrastive' in options.model: torch.save(options, './weights/options/contrastive_wiki103_50_options.pt')
        else: torch.save(options, './weights/options/wiki103_50_options.pt')
    ...
    # load the dataset
    with open(input_dir+'/vocab.pkl','rb') as f:
        vocab=pickle.load(f)
    vocab_size = len(vocab)
    if '20ng' in input_dir:
        with open(input_dir+'/dataset_20ng.pkl','rb') as f:
            datasets = pickle.load(f)
    elif 'IMDb' in input_dir:
        with open(input_dir+'/dataset_IMDb.pkl','rb') as f:
            datasets = pickle.load(f)
    else:
        with open(input_dir+'/dataset_wiki103.pkl','rb') as f:
            datasets = pickle.load(f)

    #1. training data
    if options.positive:
        positive_samples = torch.load(input_dir+"/"+options.positive)
    else:
        positive_samples = None
    training_set = datasets['train']
    options.n_train = training_set['tokens'].shape[0]
    train_X, row_selector, substitute_min_indices_tr, substitute_max_indices_tr = get_batch(training_set, range(options.n_train), vocab_size, dataset_name=dataset_name, topK=options.topk, pos_samples=positive_samples)
    ...
    train_ids = range(options.n_train)
    ...
    num_docs_valid = valid_set['tokens'].shape[0]
    dev_X, _, _, _ = get_batch(valid_set, range(num_docs_valid), vocab_size, topK=0)
    dev_labels = None
    dev_prior_covars = None
    dev_topic_covars = None
    ...
    n_test = test_set['tokens'].shape[0]
    test_X, _, _, _ = get_batch(test_set, range(n_test), vocab_size, topK=0) 
    ...
    if options.retrain:
        training_set['tokens'] = np.concatenate([training_set['tokens'], valid_set['tokens']])
        training_set['counts'] = np.concatenate([training_set['counts'], valid_set['counts']])
        options.n_train = training_set['tokens'].shape[0]
        _, _, substitute_min_indices_tr, substitute_max_indices_tr = get_batch(training_set, range(n_train), vocab_size, topK=options.topk)
    ...
    ## define checkpoint
    ckpt_file = 'seed_{}_VICNTM_{}_K_{}_std_{}_cov_{}_topK_{}'.format(seed, dataset_name, options.n_topics, options.std_coeff, options.cov_coeff, options.topk)
    options.output_dir = Path.cwd().joinpath(options.output_dir, ckpt_file)
    print(options.output_dir)
    if not os.path.exists(options.output_dir): os.makedirs(options.output_dir)
    ckpt = Path.cwd().joinpath(options.output_dir,ckpt_file)
    print(ckpt)
    ...
    model, n_of_epochs = train(model, network_architecture, dataset_name, train_X, vocab, substitute_max_indices_tr, substitute_min_indices_tr, train_labels, train_prior_covars, train_topic_covars, options.output_dir, ckpt, training_epochs=options.epochs, batch_size=options.batch_size, rng=rng, X_dev=dev_X, Y_dev=dev_labels, PC_dev=dev_prior_covars, TC_dev=dev_topic_covars)
    ...
    if dev_X is not None:
        ...
        npmi_dev = get_topic_coherence(beta, dev_X, vocab)
        print('NPMI for dev= ',npmi_dev)
        fh.write_list_to_text([str(npmi_dev)], os.path.join(options.output_dir, 'npmi.dev.txt'))
        td = get_topic_diversity(beta,10)
        print('Topic Diversity= ', td)
        fh.write_list_to_text([str(td)], os.path.join(options.output_dir, 'td.txt'))
        irbo_metric = InvertedRBO(beta=beta)
        irbo_score = irbo_metric.score()
        print('irbo= ', irbo_score)
        fh.write_list_to_text([str(irbo_score)], os.path.join(options.output_dir, 'irbo.txt'))
    if test_X is not None:
        ...
        npmi_test, topics = get_topic_coherence(beta, test_X, vocab, mode='test')
        print('NPMI for test= ',npmi_test)
        npmi_test=[str(npmi_test), str(n_of_epochs)]
        npmi_test+=topics
        fh.write_list_to_text(npmi_test, os.path.join(options.output_dir, 'npmi.test.txt'))
...
def get_batch(dataset, indices, vocab_size, dataset_name=None, topK=15, pos_samples=None):
    """
    ...
    Args:
        dataset: dict contains tokens and counts of dataset
        indices ([type]):  numpy array 
        vocab_size ([type]): [description]
        topK: number of tokens to be replaced
    Returns:
    ...
    row_sums = np.array(data_batch.sum(axis=1)).reshape((batch_size,))
    row_selector = np.array(row_sums > 0, dtype=bool)
    print("Found %d non-empty documents" % np.sum(row_selector))
    data_batch = data_batch[row_selector, :]
    

    if topK != 0:
        subs_file='data/data_{}_v5/substitute.pkl'.format(dataset_name)
        if os.path.exists(subs_file):
            with open(subs_file, 'rb') as f:
                subs=pickle.load(f)
                substitute_max_indices=subs['max']
                substitute_min_indices=subs['min']
        else:
            #apply tf-idf on data_batch
            transformer = TfidfTransformer()
            tfidf = transformer.fit_transform(data_batch)
            tfidf=torch.tensor(tfidf.toarray())
            tokens_batch = tokens[indices]
            substitute_min_indices = np.zeros([batch_size, topK], dtype=int)
            substitute_max_indices = np.zeros([batch_size, topK], dtype=int)
            for i in range(batch_size):
                tokenid_tfidf = sorted(zip(tokens_batch[i], tfidf[i][tokens_batch[i]]), key=lambda x:x[1])
                min_values = tokenid_tfidf[:topK]
                max_values = tokenid_tfidf[-topK:]
                if len(min_values) < topK or len(max_values) < topK:
                    print('number of tokens is not enough')
                substitute_min_indices[i] = [index for index, _ in min_values]
                substitute_max_indices[i] = [index for index, _ in max_values]
            with open(subs_file,'wb') as f:
                subs={'max': substitute_max_indices,'min': substitute_min_indices}
                pickle.dump(subs,f)
        if pos_samples is not None:
            return data_batch, row_selector, pos_samples, substitute_max_indices
        else:
            return data_batch, row_selector, substitute_min_indices, substitute_max_indices
    else:
        return data_batch, row_selector, None, None
...
def make_network(options, vocab_size, label_type=None, n_labels=0, n_prior_covars=0, n_topic_covars=0):
    ...
    network_architecture = \
        dict(embedding_dim=options.emb_dim,
             ...
             dist=options.dist,
             model=options.model,
             sim_coeff=options.sim_coeff,
             std_coeff=options.std_coeff,
             cov_coeff=options.cov_coeff,
             deepvic=options.deepvic,
             deepvc=options.deepvc,
             cl=options.cl
             )
    return network_architecture


def train(model, network_architecture, dataset_name, X, vocab, substitute_max_indices_tr, substitute_min_indices_tr, Y, PC, TC, output_dir, ckpt, optuna_flag=0, batch_size=200, training_epochs=500, display_step=20, X_dev=None, Y_dev=None, PC_dev=None, TC_dev=None, bn_anneal=True, init_eta_bn_prop=1.0, rng=None, min_weights_sq=1e-7):
    early_stopping = EarlyStopping(output_dir, patience=40, verbose=True, path=ckpt, optuna_flag = optuna_flag)
    # Train the model
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            # get a minibatch
            batch_xs, batch_ys, batch_pcs, batch_tcs, idx = next(mb_gen)
            batch_subs_max_idx_tr = substitute_max_indices_tr[idx]
            batch_subs_min_idx_tr = substitute_min_indices_tr[idx]
            # do one minibatch update
            if cl != 0:
                cost, recon_y, thetas, nl, kld, cl_loss, reg_loss = model.fit(batch_xs, batch_ys, batch_pcs, batch_tcs, batch_subs_max_idx_tr, batch_subs_min_idx_tr, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci, cl=1)
            else:
                cost, recon_y, thetas, nl, kld, reg_loss, repr_loss, std_loss, cov_loss = model.fit(batch_xs, batch_ys, batch_pcs, batch_tcs, batch_subs_max_idx_tr, batch_subs_min_idx_tr, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci, cl=0)
            ...
            avg_reg += float(reg_loss) / n_train * batch_size
            if cl != 0:
                avg_cl += float(cl_loss) / n_train * batch_size
            if cl == 0:
                avg_repr += float(repr_loss) / n_train * batch_size
                avg_std += float(std_loss) / n_train * batch_size
                avg_cov += float(cov_loss) / n_train * batch_size
            batches += 1
            if np.isnan(avg_cost):
                ...
                sys.exit()
        if cl == 0:
            plot_for_repr_loss.append(avg_repr)
            plot_for_std_loss.append(avg_std)
            plot_for_cov_loss.append(avg_cov)
        else:
            plot_for_cl_loss.append(avg_cl)
        ...
        if epoch % display_step == 0 and epoch > 0:
            if network_architecture['n_labels'] > 0:
                ...
                if cl!=0:
                    print("Epoch:", '%d' % epoch, "cost=", "{:.9f}".format(avg_cost), "nl=", "{:.9f}".format(avg_nl), "kld=", "{:.9f}".format(avg_kld), "cl=", "{:.9f}".format(avg_cl),"reg=", "{:.9f}".format(avg_reg),)
                else:
                    print("Epoch:", '%d' % epoch, "cost=", "{:.9f}".format(avg_cost), "nl=", "{:.9f}".format(avg_nl), "kld=", "{:.9f}".format(avg_kld), "reg=", "{:.9f}".format(avg_reg),)
        if X_dev is not None:
            # switch to eval mode for intermediate evaluation
            model.eval()
            with torch.no_grad():
                beta = model.get_weights()
                npmi = get_topic_coherence(beta, X_dev, vocab)
                plot_val_npmi.append(npmi)
                if epoch % display_step == 0:
                    print('NPMI for now= ',npmi)
            ...
            model.train()
            if dataset_name=='IMDb':n_burn_in=179
            else:n_burn_in=139
            if epoch >= n_burn_in:
                early_stopping(npmi, model, plot_val_npmi, plot_for_repr_loss, plot_for_std_loss, plot_for_cov_loss, plot_for_cl_loss)
        ...
        if bn_anneal:
            if eta_bn_prop > 0:
                eta_bn_prop -= 1.0 / float(0.75 * 200)
                if eta_bn_prop < 0:
                    eta_bn_prop = 0.0

    # finish training
    model = torch.load(open(ckpt, 'rb'))
    model.eval()
    return model, n_of_epochs
...
def create_minibatch(X, Y, PC, TC, batch_size=200 , rng=None):
    ...
    while True:
        ...
        yield X_mb, Y_mb, PC_mb, TC_mb, ixs
...
def get_topic_coherence(beta, data, vocab, mode='val'):
    D = data.shape[0]
    if mode=='test':
        print('D: ', D)
    TC = []
    ...
    for k in range(num_topics):
        ...
        TC_k = 0
        npmi_vals=[]
        for i, word1 in enumerate(top_10):
            if top_words[i] not in vocab:
                index1 = None
            else:
                index1 = word1
            for j, word2 in enumerate(top_10[i+1:]):
                if top_words[j] not in vocab:
                    index2 = None
                else:
                    index2 = word2
                if index1 is None or index2 is None:
                    ...
                    col1 = np.array(data[:,index1]>0,dtype=int)
                    col2 = np.array(data[:,index2]>0,dtype=int)
                    c1 = col1.sum()
                    c2 = col2.sum()
                    ...
                    if c12 == 0:
                        ...
                        npmi = (np.log10(D) + np.log10(c12) - np.log10(c1) - np.log10(c2)) / (np.log10(D) - np.log10(c12))
                npmi_vals.append(npmi)
        if mode=='test':
            topics.append(str(np.mean(npmi_vals)) + ': ' + ' '.join(top_words))
        npmi_means.append(np.mean(npmi_vals))
    # print(np.mean(npmi_means))
    if mode=='test':
        return np.mean(npmi_means), topics
    else:
...
def evaluate_perplexity(model, X, Y, PC, TC, batch_size, eta_bn_prop=0.0):
    ...
    # print(f"losses length: {len(losses)} | {losses}")
    # print(f"{losses.sum()} / {doc_sums.sum()}")
    # print(losses)
    perplexity = np.exp(losses.sum() / doc_sums.sum())
    return perplexity
