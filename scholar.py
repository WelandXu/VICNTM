...
class Scholar(object):
    ...
    def fit(self, X, Y, PC, TC, subs_max_idx, subs_min_idx, eta_bn_prop=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        """
        ...
        # do a forward pass
        thetas, X_recon, Y_probs, losses, cl_loss, reg_loss = self._model(X, Y, PC, TC, subs_max_idx, subs_min_idx, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci)
        loss, nl, kld = losses
        # update model
        loss.backward()
        self.optimizer.step()
        ...
        if cl_loss is not None:
            ...
            return loss.to('cpu').detach().numpy(), Y_probs, thetas.to('cpu').detach().numpy(), nl.to('cpu').detach().numpy(), kld.to('cpu').detach().numpy(), reg_loss.to('cpu').detach().numpy()
...
class torchContrastiveScholar(nn.Module):
    ...
    def forward(self, X, Y, PC, TC, subs_max_idx=None, subs_min_idx=None, compute_loss=True, do_average=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        if subs_max_idx is not None:
            ...
            if self.projector is None or self.deepvc > self.deepvic:
            ...
            if self.projector is not None:
                ...
                if self.deepvic > self.deepvc:
                    #deep vicreg
                    vcreg_loss = self.vicreg(normalized_Z, normalized_positive_Z)
