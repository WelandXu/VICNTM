...
data_dir='./data_wiki_v3/wikitext-103'
...
with open('./compare_vocabs/wiki/vocabulary.txt','r') as f:
    vocab = f.read().split('\n')
    print(vocab[0])
word2id = dict([(w, j) for j, w in enumerate(vocab)])
id2word = dict([(j, w) for j, w in enumerate(vocab)])
...
path_save = './wiki_min_df_' + str(min_df) + '/'
...
# output_file = "wiki_for_octis.tsv"
...
# with open('vocabulary_octis_wiki.txt', "w") as f:
#     f.write('\n'.join(vocab_words))

# print(f"vocabulary_octis_wiki.txt successfully created.")
...
# with open('wiki_docid_tr.pkl','wb') as f:
